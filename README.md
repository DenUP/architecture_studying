...
- Сделал отдельно загрузочный экран, он будет определять куда перенаправлять пользователя
- Даже после авторизации, он так же будет учитывать куда перенаправлять пользователя
- Сделал для Loader отдельную навигацию, чтобы каждрый раз можно было переиспользовать код
